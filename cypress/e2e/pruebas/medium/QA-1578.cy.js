...
describe('Test QA-1578 E2E_Recepciones_Detalle del pedido_Diferidos_Añadir diferido_Iniciar recepción_Finalizar', () => {
    ...
    it('Step 7', () => {    //Nos colocamos en la pestaña Diferidos
        cy.contains("Diferidos").click()
        cy.wait(1000)
    })

    it('Step 8',()=>{   // Añadimos intervencion
        cy.xpath("//button[contains(.,'Añadir diferido')]").click()
        cy.get(':nth-child(4) > .col > .row > .mat-form-field > .mat-form-field-wrapper > .mat-form-field-flex > .mat-form-field-infix').type("Test diferido")
        ...
        cy.get('.cdk-column-quantityTime > .mat-form-field > .mat-form-field-wrapper > .mat-form-field-flex > .mat-form-field-infix > input').clear().type('2').wait(2000)
        cy.get('.col-12 > .btn__primary').click()
        cy.xpath("//span[contains(@class,'mat-button-wrapper')]").click()
        cy.xpath("//div[contains(@class,'mat-calendar-body-cell-content mat-focus-indicator mat-calendar-body-today')]").click()
        cy.xpath("//div[contains(@class,'mat-calendar-body-cell-content mat-focus-indicator mat-calendar-body-today')]").click()
        cy.xpath("//button[contains(.,'Diferir')]").click().wait(2000)
        cy.xpath("//button[contains(.,'Iniciar recepción')]").click()  
    })

    it('Step 9', () => {   // Actualizamos km y aceptamos
        cy.xpath("//button[contains(.,'Aceptar')]").click() 
    })

    it('Step 10',()=>{   // Rellenamos todas las pestañas 
