...
describe('QA-2547 - Calendario_Detalle evento personal_Vista rápida_Ir al detalle', () => {
  ...
  it('step 5', () => {
    cy.get('.btn__primary').eq(0).click()
    cy.wait(1000)
    cy.get('one-dialog-structure').should('be.ok')
    cy.wait(1000)
    cy.get('one-dialog-structure > :nth-child(1) > .col').should('have.text', 'Detalle evento personal')
    cy.wait(1000)
  });
