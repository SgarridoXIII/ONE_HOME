...
describe('Test QA-1598 E2E_Recepciones_Detalle del pedido_Recepción_Reasignar recepción_Iniciar recepción_Finalizar', () => {
    ...
    it('Step 7',()=>{   //Desde la pantalla recepción, pulsamos reasignar recepción

        cy.contains("Recepción").click()
        cy.xpath("//button[contains(.,'Reasignar recepción')]").click().wait(1000)
        cy.xpath("//input[@placeholder='Selecciona un asesor...']").click()
        cy.xpath("//span[contains(.,'Cualquier asesor')]").click().wait(3000)
        cy.xpath("//div[@class='appointment-hours__hour ng-star-inserted']").first().click()
        cy.xpath("//button[contains(.,'Guardar')]").scrollIntoView().click().wait(3000)
        cy.get(".cdk-overlay-container").then(el =>{
            if(el.text().includes("Recepción modificada")){
                cy.xpath("//button[@class='btn__secondary mr-4'][contains(.,'Cancelar')]").click().wait(2000)
            }else{
                cy.wait(2000)
            }
        })
    ...
    it('Step 8',()=>{    //Iniciamos recepcion
        cy.xpath("//button[contains(.,'Iniciar recepción')]").click()    
    })

    it('Step 9',()=>{    // Actualizar km
        cy.xpath("//button[contains(.,'Aceptar')]").click().wait(1000)
    })

    it('Step 10',()=>{   // Rellenamos pestañas y pulsamos finalizar
