...
describe('QA-2231 - E2E_Ampliaciones pedidos', () => {
  ...
  // Desde la pantalla  principal "Home", pulsamos en el botón "Ampliaciones pedidos"
  it('step 3', () => {
  ...
  // Rellenamos el campo "Cliente"
  it('step 4', () => {
    cy.get('input[data-placeholder="Buscar cliente..."]').type(cliente).type('{enter}')
    cy.wait(10000)
    cy.get('mat-option').eq(0).click()
    cy.wait(1000)
    cy.get('one-card-service').should('include.text', cliente)
    cy.wait(1000) 
    cy.get('mat-icon').should('have.text', 'close').click()
    cy.wait(1000)
  });
  
  // Rellenamos el campo Vehículo
  it('step 5', () => {
    cy.get('.mat-form-field-flex').eq(2).click()
    cy.wait(1000)
    cy.get('one-vehicle-selector').find('input[formcontrolname="search"]').type(matricula)
    cy.wait(1000)
    cy.get('one-card-vehicle').eq(0).click()
    cy.wait(1000)
    cy.get('one-card-service').should('include.text', matricula)
    cy.wait(1000)
  });

  // Pulsamos sobre uno de los resultados
  it('step 6', () => {
    cy.get('one-card-service').eq(0).click()
    cy.wait(1000)
    cy.get('one-detail-repair-order').should('be.ok')
    cy.wait(1000)
  });
  
  // Pulsamos sobre el botón "Volver"
  it('step 7', () => {
