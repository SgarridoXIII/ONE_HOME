...
describe('QA-1348 - Recepciones_Detalle del pedido_Diferidos_Eliminar_Cancelar', () => {
  ...
  // Pulsamos sobre los 3 puntos de una de las intervenciones
  it('step 6', () => {
    ...
    cy.get('one-card-package').eq(0).find('mat-icon').click({force: true})
    cy.wait(1000)
  });
  
  // Pulsamos Eliminar
  it('step 7', () => {
    cy.get('div[class="cdk-overlay-pane"]').find('button').eq(2).click()
    cy.wait(1000)
    cy.get('mat-dialog-container').should('be.ok')
    cy.wait(1000)
    cy.get('mat-dialog-container').find('h4').should('have.text', 'Eliminar intervención')
    cy.wait(1000)
  });
  ...
  it('step 8', () => {
    ...
    cy.wait(1000)
    cy.get('one-detail-repair-order').find('h5').should('have.text', 'Diferidos añadidos')
    cy.wait(1000)
  });
