...
describe('QA-1506 - Recepciones_Detalle del pedido_Ampliaciones/CAM_Ampliaciones pedido_Añadir intervención_Cambio Tipo de intervención_Aceptar', () => {
  ...
  // Desde la pestaña Ampliaciones pedido, pulsamos el botón Añadir intervención
  it('step 6', () => {
  ...
  // Pulsamos sobre la palabra Tipo de intervención de la intervención
  it('step 8', () => {
    cy.get('one-card-package-expansion-v2-header-title').find('mat-label').eq(1).click()
    cy.wait(1000)
    cy.get('mat-dialog-container').should('be.ok')
    cy.wait(1000)
    cy.get('h4').should('have.text', 'Informar tipo de Intervención')
    cy.wait(1000)
  });
  
  // Editamos el tipo de intervención de la intervención (Ninguno, Inspección y mantenimiento,
  // Diagnosis, Medida de servicio, Eletrónica de confort, Motores, Transmisión de fuerza, 
  // Tren de rodaje, Climatización, Chapa y pintura) y pulsamos Aceptar
  it('step 9', () => {
    cy.get('mat-select[formcontrolname="taskType"]').click()
    cy.wait(1000)
    ...
    cy.wait(1000)
    cy.get('.mat-snack-bar-container').should('have.text', 'Tipo de intervención actualizada correctamente')
    cy.wait(1000)
  });
