...
describe('QA-1220 - Recepciones_Buscador_Seleccionar Cliente', () => {
  ...
  // Pulsamos sobre el campo Cliente y escribimos el nombre de un cliente
  it('step 4', () => {
    cy.get('input').eq(1).click()
    cy.wait(1000)
    cy.get('input').eq(1).clear().type(`${cliente}{enter}`)
    cy.wait(5000)
  });
  
  // Seleccionamos un cliente pulsando sobre la flecha >
  it('step 5', () => {
  ...
  it('step 6', () => {
    cy.get('input').eq(1).should('include.value', cliente)
    cy.wait(1000)
  });
