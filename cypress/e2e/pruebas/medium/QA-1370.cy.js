...
describe('QA-1370 - Recepciones_Detalle del pedido_Checklist_Lunas_Añadir otras intervenciones', () => {
  ...
  it('step 7', () => {
    cy.get(':nth-child(1) > .checklistItems__checklistItem__statuses > .checklistItems__checklistItem__statuses__status--yellow').click()
    cy.wait(1000)
    cy.get('one-dialog-structure').find('svg[class="icon ng-star-inserted"]').click()
    cy.wait(1000)
  });

  // Cambiamos uno de los módulos de color rojo a amarillo o viceversa
  it('step 8', () => {
    cy.get(':nth-child(1) > .checklistItems__checklistItem__statuses > .checklistItems__checklistItem__statuses__status--red').click()
    cy.wait(1000)
  });
  
  // Añadimos manualmente una intervención en el campo Añadir intervención y pulsamos Añadir
  it('step 9', () => {
    cy.get('input[formcontrolname="createPackage"]').type(`test${generateRandomNumber()}`)
    cy.wait(1000)
    cy.get('button[class="btn__secondary mb-3"]').click()
    cy.wait(1000)
  ...
  // Rellenamos los apartados correspondientes de la intervención (recambios, mano de obra y fluidos)
  it('step 10', () => {
    cy.get('.mat-snack-bar-container').should('be.ok')
    cy.wait(1000)
    cy.get('.mat-snack-bar-container').should('have.text', 'Intervenciones actualizadas correctamente')
    cy.wait(1000)
  });
