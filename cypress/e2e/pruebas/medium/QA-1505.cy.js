...
describe('QA-1505 - Recepciones_Detalle del pedido_Ampliaciones/CAM_Ampliaciones pedido_Añadir intervención_Cambio pagador_Aceptar', () => {
  ...
  // Pulsamos sobre la palabra Pagador de la intervención
  it('step 8', () => {
    cy.get('one-card-package-expansion-v2-header-title').find('mat-label').eq(0).click()
    cy.wait(1000)
    cy.get('mat-dialog-container').should('be.ok')
    cy.wait(1000)
    cy.get('h4').should('have.text', 'Informar tipo de Pagador')
    cy.wait(1000)
  });
  
  // Cambiamos el pagador de la intervención (Ninguno, Interno, 
  // Aseguradora, Cliente propietario, Empresa Renting, Garantías, VWFS) y pulsamos Aceptar
  it('step 9', () => {
    cy.get('mat-select[formcontrolname="payerType"]').click()
    cy.wait(1000)
    ...
    cy.wait(1000)
    cy.get('.mat-snack-bar-container').should('have.text', 'Pagador actualizado correctamente')
    cy.wait(1000)
  });
