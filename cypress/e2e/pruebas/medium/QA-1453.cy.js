...
describe('QA-1453 - Recepciones_Detalle del pedido_Multimedia_Añadir documento', () => {
  ...
  // No seleccionamos ningún documento
  it('step 7', () => {
    cy.get('button[class="btn__primary w-100 ng-star-inserted"]').should('have.attr', 'disabled')
    cy.wait(1000)
  });
  
  // Seleccionamos un documento pero no rellenamos el campo Comentarios
  it('step 8', () => {
    cy.get('.fileInput').attachFile(pdf)
    cy.wait(1000)
    cy.get('button[class="btn__primary w-100 ng-star-inserted"]').should('not.have.attr', 'disabled')
    cy.wait(1000)
  });
