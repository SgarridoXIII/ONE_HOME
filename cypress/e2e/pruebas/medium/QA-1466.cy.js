...
describe('QA-1466 - Recepciones_Detalle del pedido_Multimedia_Editar vídeo', () => {
  ...
  // Pulsamos sobre uno de los vídeos
  it('step 6', () => {
    ...
    cy.wait(1000)
    cy.get('mat-select[formcontrolname="id"]').click()
    cy.wait(1000)
    cy.get('mat-option').eq(0).click()
    cy.wait(1000)
    cy.get(':nth-child(5) > .btn__primary').click()
    cy.wait(1000)
    cy.get('one-video').eq(0).click()
    cy.wait(1000)
  });
  
  // Pulsamos el botón Cambiar vídeo y seleccionamos un vídeo, y/o editamos el campo Comentarios y pulsamos Guardar
  it('step 7', () => {
    cy.get('.pl-3 > .btn__primary').click()
    cy.wait(1000)
    cy.get('.fileInput').attachFile({
      filePath: video,
      mimeType: 'video/mp4',
    ...
    cy.wait(1000)
    cy.get('textarea[formcontrolname="comments"]').type('VideoEdit')
    cy.wait(1000)
    cy.get(':nth-child(5) > .btn__primary').click()
    cy.wait(1000)
