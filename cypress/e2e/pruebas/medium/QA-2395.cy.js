...
describe('QA-2395 - Gestionar diferidos_Cliente', () => {
  ...
  it('step 3', () => {
    ...
    cy.wait(1000)
    cy.get('one-deferreds').should('be.ok')
    cy.wait(1000)
  });
  
  // Hacemos click en el campo "Cliente"
  it('step 4', () => {
    cy.get('input[formcontrolname="customer"]').click()
    cy.wait(1000)
    cy.get('one-customer-selector').should('be.ok')
    cy.wait(1000)
    cy.get('h4').should('have.text', 'Seleccionar cliente')
    cy.wait(1000)
  });
  ...
  it('step 5', () => {
    ...
    cy.get('input[formcontrolname="customer"]').then(e => {
      expect(e.val()).not.null
      cliente = e.val()
      cy.wait(1000)

      cy.get('one-card-package-deferred').should('include.text', cliente)
      cy.wait(1000)
    })
  });
