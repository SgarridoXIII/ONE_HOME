...
var nuevaReferencia = 'B000PRUEBAS';
let antiguaReferencia = ''


describe('QA-1074 - Precios_Pestaña Fluidos_Editar_Campo Referencia', () => {
  ...
  //Editamos el campo Referencia
  it('step 7', () => {
    cy.get('#mat-input-15').then(e => {
        expect(e.val()).not.null
        antiguaReferencia = e.val()
    })
    cy.wait(1000)
    cy.get('#mat-input-15').clear().type(nuevaReferencia)
    cy.contains('mat-header-cell','Referencia').click()
    cy.wait(3000)
    cy.get('.mat-snack-bar-container').should('be.ok')
  });
  
  //Comprobamos que el campo Referencia se ha editado correctamente
  it('step 8', () => {
    cy.get('#mat-input-15').should('contain.value',nuevaReferencia)
    cy.get('#mat-input-15').clear().type(antiguaReferencia)
    cy.contains('mat-header-cell','Referencia').click()
    cy.wait(3000)
  });
