...
describe('QA-1463 - Recepciones_Detalle del pedido_Multimedia_Editar documento', () => {
  ...
  // Añadimos un documento en formato pdf
  it('step 6', () => {
    cy.get('svg[class="mat-tooltip-trigger icon-doc ml-2 pointer"]').click()
    cy.wait(1000)
    cy.get('.fileInput').attachFile(pdf)
    cy.wait(1000)
    cy.get(':nth-child(3) > .btn__primary').click()
    cy.wait(1000)
  ...
  // Editamos el documento y/o el campo comentarios y pulsamos Guardar
  it('step 7', () => {
    cy.get('one-card-doc').eq(0).click()
    cy.wait(1000)
    cy.get('textarea[formcontrolname="comments"]').clear().type('TestDocument')
    cy.wait(1000)
    cy.get(':nth-child(3) > .btn__primary').click()
    cy.wait(1000)
  });
  
  // Descargamos el documento subido previamente
  it('step 8', () => {
    cy.get('svg[matTooltip="Descargar"]').eq(0).click()
    cy.wait(1000)
    cy.verifyDownload('pdf.pdf');
    cy.wait(1000)
  });
