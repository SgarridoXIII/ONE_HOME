...
describe('QA-2426 - E2E_Calendario_CrearNuevaCita_CitaConCliente_Semana', () => {
  ...
  // Hacemos click en un día de la semana sobre la hora que queramos
  it('step 4', () => {
    cy.get('.calendar-view_mode').eq(2).should('have.text', ' Semana ').click()
    cy.wait(1000)
    
    // Eliminar
    cy.get('.calendar-navigation__icon').eq(1).click()
    cy.wait(1000)
    // Eliminar

    cy.get('.cal-day-column.ng-star-inserted').children('.cal-hour.ng-star-inserted').eq(2).click()
    cy.wait(1000)
  });
