...
describe('QA-1487 - Recepciones_Detalle del pedido_Ampliaciones/CAM_Ampliaciones pedido_Añadir intervención_Cambio estado_Aceptar', () => {
  before(() => {
  ...
  // Pulsamos sobre la palabra Estado de la intervención
  it('step 8', () => {
    cy.get('one-card-package-expansion-v2-header-title').find('mat-label').eq(2).click()
    cy.wait(1000)
    cy.get('mat-dialog-container').should('be.ok')
    cy.wait(1000)
    cy.get('h4').should('have.text', 'Informar estado')
    cy.wait(1000)
  });
  
  // Cambiamos el estado de la intervención a: Pte respuesta cliente y pulsamos Aceptar
  it('step 9', () => {
    cy.get('mat-select[formcontrolname="packageStatus"]').click()
    cy.wait(1000)
    cy.get('mat-option').eq(0).click()
    cy.wait(1000)
  ...
  // Cambiamos el estado de la intervención a: Autorizada y pulsamos Aceptar
  it('step 10', () => {
    cy.get('one-card-package-expansion-v2-header-title').find('mat-label').eq(2).click()
    cy.wait(1000)
    cy.get('mat-dialog-container').should('be.ok')
  ...
  // Cambiamos el estado de la intervención a: Descartada y pulsamos Aceptar
  it('step 11', () => {
    cy.get('.btn__secondary').contains('Añadir intervención').click()
    cy.wait(1000)
