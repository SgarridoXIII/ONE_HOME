...
describe('QA-811 - E2E_Crear cliente_Empresa_Usuario activo', () => {
  ...
  //En la pantalla "Crear cliente" rellanamos los campos correspondientes del formulario, el campo "Usuario activo" activo. Pulsamos en el botón "Crear cliente".
  it('step 5', () => {
    ...
    cy.get('input[id="mat-slide-toggle-2-input"]').then(e => {
      expect(e.attr('aria-checked')).to.equal(activeState)
      expect(e.attr('aria-checked')).not.to.equal(desactiveState)
    })
    cy.contains('button','Crear cliente').click()
  });
